...
function renderBlog() {
  ...
  const matches = (post) => {
    ...
    const haystack = `${post.title} ${post.excerpt} ${post.category} ${(post.content || []).map((section) => (Array.isArray(section.paragraphs) ? section.paragraphs : []).join(' ')).join(' ')}`.toLowerCase();
